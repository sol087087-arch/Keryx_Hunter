...
_CHECKPOINT_EVERY_N = 1 # inside the loop = every iteration, as suggested
# ---------------------------------------------------------------------------
# Metrics
# ---------------------------------------------------------------------------
@dataclass
...
        logger.info(f"[Orchestrator] Initialized | models={len(available_models)} | GPU={has_gpu}")
    # ------------------------------------------------------------------
    # Public entry point
    # ------------------------------------------------------------------
    async def hunt(
...
                # Checkpoint inside loop — suggested fix, agreed and implemented
                await self._save_checkpoint(target_path)
                continue
            # Checkpoint after every iteration — power loss at level 3 loses nothing
            await self._save_checkpoint(target_path)
...
        1. Explicit --mode airgapped → always True, no network test.
        2. Proxy env vars present → assume connectivity (disputed: proxy can be
           broken, but checking is complex; treating its presence as a signal
           is a reasonable heuristic with a clear comment).
        3. Real TCP probe to multiple endpoints in parallel — first success wins.
        DISPUTE RESOLVED on proxy check:
        We keep it but document the limitation — proxy presence is a heuristic,
        not a guarantee. If proxy is set but broken, we'll fall through to the
        TCP test which will fail and correctly report air-gapped.
        """
        if explicit_mode == "airgapped":
            return True
        # Heuristic: proxy vars suggest someone configured network access.
        # Note: broken proxy will still fail the TCP test below.
        if os.environ.get("HTTP_PROXY") or os.environ.get("HTTPS_PROXY"):
            logger.debug("[Orchestrator] Proxy env var detected — assuming network available")
            return False
        # Parallel TCP probe — multiple targets, first success = has internet.
        # DISPUTE RESOLVED on asyncio.timeout: requires Python 3.11+.
        # This file declares python_requires >= 3.11 in pyproject.toml.
        probes = [
...
        return Path(".keryx_checkpoints") / f"orchestrator*{key}.json"
    async def _save_checkpoint(self, target_path: str) -> None:
        """
        Async checkpoint save — json.dump offloaded to thread pool
        so it doesn't freeze the event loop when context grows large.
        AGREED: async write is the right call. ensure_ascii=False added
        so Cyrillic hypotheses are human-readable in the checkpoint file.
        """
...
        """Atomic write: .tmp → rename. Never corrupts existing checkpoint."""
...
        """
        Run models in parallel with a global timeout.
        AGREED: asyncio.timeout() as outer guard prevents one hung model
        from blocking the entire swarm. Requires Python 3.11+.
        DISPUTE RESOLVED on vote keys: MD5 8-char hex → truncated text (60 chars).
        Saves memory equally well, readable in logs and checkpoint files.
        """
