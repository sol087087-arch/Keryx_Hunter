...
    model_name: str
    hypothesis: str
    verdict: bool # True = real/exploitable
    confidence: float
    reasoning: str = ""
    weight: float = 1.0
...
    hypotheses: List[str]
    confirmed: List[str] # reached consensus
    rejected: List[str] # explicitly rejected
    votes: Dict[str, List[SwarmVote]] # hyp → ALL votes (+ and -)
...
    grammar_violations: int = 0 # should always be 0 with GBNF
@dataclass
class ModelWeight:
    model_name: str
    weight: float = 1.0
...
    "gpt-4o": ModelWeight("gpt-4o", 1.35, 98),
    "gpt-4-turbo": ModelWeight("gpt-4-turbo", 1.40, 99),
    "llama-70b": ModelWeight("llama-70b", 1.25, 90),
    "qwen-32b": ModelWeight("qwen-32b", 1.20, 88),
    "deepseek-coder": ModelWeight("deepseek-coder", 1.15, 85),
    "llama-13b": ModelWeight("llama-13b", 0.90, 70),
    "llama-8b": ModelWeight("llama-8b", 0.75, 55),
    "phi-3-mini": ModelWeight("phi-3-mini", 0.60, 40),
    "gemma-2b": ModelWeight("gemma-2b", 0.40, 25),
}
# ---------------------------------------------------------------------------
# SwarmDebate
# ---------------------------------------------------------------------------
...
        max_concurrent: int = 2, # FIX 7: was hardcoded, now configurable
    ):
        self.models = models
...
        # Semaphore prevents OOM when multiple large local models generate concurrently
...
        frontier = {"claude-3.5-sonnet", "gpt-4o", "gpt-4-turbo"}
...
        # Adversarial skeptic pass over confirmed candidates
...
        result.grammar_violations = 0 # GBNF guarantees this
        logger.info(
...
        # FIX 3: collect ALL votes (True and False), not just positive ones
        # This is required for correct raw_ratio and reject logic.
...
                votes[hyp].append(vote) # append regardless of verdict
...
            round_used = rnd
            tasks = [
                self.*analyze_with_model_iterative(m, hypotheses, context, history, rnd)
...
            *, _, w_ratio, _ = self._compute_consensus(all_votes, hypotheses, len(self.models))
...
        )
    # ------------------------------------------------------------------
    # Single-model analysis — FIX 1 + FIX 4
    # ------------------------------------------------------------------
...
                # FIX 4: local.py generate_async has no config= param.
                # Unpack GenerationConfig fields into individual kwargs.
                # FIX 1: generate_async returns str, not GenerationResult.
...
    ) -> int:
        overrides = 0
        # Iterate over a copy since we modify result.confirmed
...
        )
    # ------------------------------------------------------------------
    # Parsing — GBNF guarantees valid JSON; strip fences only as safety net
    # FIX 6: comment now honest — fences shouldn't appear but we strip them
    # defensively in case the backend didn't enforce grammar (e.g. remote API)
    # ------------------------------------------------------------------
...
    ) -> Dict[str, SwarmVote]:
        raw = raw.strip()
        # Defensive fence stripping for non-grammar-enforced backends (cloud APIs)
        if raw.startswith(""):             parts = raw.split("")
if len(parts) >= 2:
raw = parts[1].lstrip("json").strip()
data = json.loads(raw) # raises JSONDecodeError if truly invalid
...
# Fill any missing hypotheses with explicit False vote
for hyp in hypotheses:
if hyp not in votes:
votes[hyp] = SwarmVote(
model_name=model_name, hypothesis=hyp,
verdict=False, confidence=0.0,
reasoning="not_evaluated", weight=weight,
response_time_ms=response_time_ms,
)
return votes
def _neutral_votes(
...
return {
h: SwarmVote(model_name=model_name, hypothesis=h,
verdict=False, confidence=0.0,
reasoning=reason, weight=w)
for h in hypotheses
}
# ------------------------------------------------------------------
# Consensus computation — uses ALL votes
# ------------------------------------------------------------------
...
# FIX 3: raw ratio now uses all votes, not just positive ones
...
# FIX 8: ASCII markers instead of Unicode — safe on all terminals
lines = [
...
async def verify_hypothesis_with_swarm(
hypothesis: str,
models: List[ModelInterface],
context: Optional[str] = None,
consensus_threshold: float = 0.67,
skeptic_model: Optional[ModelInterface] = None,
) -> Tuple[bool, float, SwarmResult]:
"""
Verify a single hypothesis with the full swarm.
FIX 5: confidence was weighted_consensus_ratio (overall average across all
hypotheses). For single-hypothesis case we return per-hypothesis confidence:
the average confidence of all models that voted True. Falls back to ratio.
Returns:
(is_confirmed, confidence, full_result)
"""
debate = SwarmDebate(
models=models,
consensus_threshold=consensus_threshold,
...
result = await debate.debate([hypothesis], context)
is_confirmed = hypothesis in result.confirmed
# Per-hypothesis confidence: mean confidence of positive voters
hyp_votes = result.votes.get(hypothesis, [])
pos_votes = [v.confidence for v in hyp_votes if v.verdict]
confidence = sum(pos_votes) / len(pos_votes) if pos_votes else result.weighted_consensus_ratio
return is_confirmed, confidence, result
