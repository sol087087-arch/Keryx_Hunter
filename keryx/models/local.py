...
class GenerationResult:
    __slots__ = ("text", "tokens_generated", "tokens_per_second", "duration_ms")

    def __init__(self, text: str, tokens_generated: int, tokens_per_second: float, duration_ms: float):
        self.text = text
...
class ToolDefinition:
    __slots__ = ("name", "description", "parameters", "executor")
...
class LocalModel(ModelInterface):
    ...
    def clear_kv_cache(self, keep_system_tokens: int = 0) -> None:
        try:
            if keep_system_tokens > 0:
                ...
                self.llm.reset()
                logger.debug("[LocalModel] KV cache cleared fully")
    ...
    # ------------------------------------------------------------------
    # ModelInterface abstract method implementations
    # ------------------------------------------------------------------
    ...
    def generate_result(
        ...
        tokens_in = len(self.llm.tokenize(prompt.encode()))
        tokens_out = len(self.llm.tokenize(text.encode()))
    ...
    def get_capabilities(self) -> ModelCapabilities:
        return ModelCapabilities(
            ...
            supports_speculative=self._draft_llm is not None,
            supports_min_p=False,
            requires_gpu=self.config.n_gpu_layers != 0,
            is_local=True,
            is_quantized=True,
        )
    ...
    def get_memory_usage(self) -> Dict[str, float]:
        ...
        proc = psutil.Process()
        result.update({
            "ram_used_gb": proc.memory_info().rss / 1024 ** 3,
