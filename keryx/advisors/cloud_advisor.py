...
# - Local digest model (8B summarizes before sending to cloud)
...
_SCRUB_PATTERNS: list[tuple[str, str]] = [
    ...
    (r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b', 'user@example.com'),
...
class CloudAdvisor(BaseAdvisor):
    ...
    Privacy stack:
    1. Optional local digest: cheap 8B model summarizes history
    2. Scrubbing: paths, IPs, AWS keys, emails replaced before cloud call
    Reliability stack:
    ...
    def _check_preflight(self) -> tuple[bool, str]:
        ...
        if self.budget_limit_usd and self._session_cost_usd >= self.budget_limit_usd:
            return False, f"budget_exhausted_${self._session_cost_usd:.2f}"
    ...
    async def advise(self, context: Any) -> AdvisorResponse:
        ok, reason = self._check_preflight()
        if not ok:
            return self._fallback_response(context, f"preflight_{reason}")
    ...
    async def _build_secure_prompt(self, context: Any, available_tools: list[str]) -> str:
        ...
        blacklisted = list(getattr(context, "blacklist_hypotheses", []))
        current_hyps = [
    ...
    async def _call_with_retry(self, prompt: str) -> str:
        ...
        for attempt in range(1, self.retry_config.max_attempts + 1):
            try:
                ...
                wait = delay + jitter
                logger.warning(
                    f"[{self.name}] Attempt {attempt} failed ({exc.__class__.__name__}). "
