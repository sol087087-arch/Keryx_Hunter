...
class CascadeAdvisor(BaseAdvisor):
    ...
    5. On CRITICAL error → disable advisor for rest of session
    ...
    async def advise(self, context: Any) -> AdvisorResponse:
        ...
        for i, state in enumerate(self._chain):
            ...
            # Skip local-only advisors on critical confidence
            ...
            try:
                ...
                if severity == ErrorSeverity.CRITICAL and self.disable_on_critical:
                    state.disabled = True
                    logger.error(f"[{self.name}] Disabled '{advisor.name}' for session.")
