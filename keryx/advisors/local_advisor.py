...
    FIX 4: previous version used str.replace() on a substring that didn't
    exist in BASE_ADVISOR_GRAMMAR → silent no-op, grammar was never changed.
    Now we construct the tool rule separately and substitute the placeholder.
    """
...
    # TOOL_RULE is the substitution target — explicit and unambiguous
...
        """
        Derive key from context state.
        FIX 7: previous version called str(step) on step objects, which
        may produce memory addresses ("object at 0x...") — unstable across
        runs. We now extract serializable fields explicitly.
        """
...
    - Context-aware fallback: different advice depending on failure mode
    - Deduplication: repeated identical advice is flagged
    """
    name = "local-advisor"
...
        """
        Trigger on: low confidence, too many parse errors, or stalled hunt.
        FIX 5: previous anti-spam logic used self._call_count (undefined)
        and silently blocked low-confidence re-triggers after first call.
        Now: use self.calls_made (from BaseAdvisor), and allow re-trigger
        on low confidence up to max_calls_per_session without artificial cap.
        """
...
        # KV cache deduplication hint (informational — no API call)
...
        try:
            # FIX 2: generate_async returns str, not GenerationResult
            # FIX 3: generate_async has no config= param — pass grammar and max_tokens directly
...
"""
Persist blacklisted hypotheses back into context.
FIX 6: never mutate context.blacklisted_hypotheses directly via append.
SharedContext may not own this field, and direct mutation breaks
checkpoint serialization. Use context.blacklist_hypothesis() if available,
otherwise update via dedicated setter.
"""
...
# Do NOT mutate context attributes directly
# ------------------------------------------------------------------
# Fallback
# ------------------------------------------------------------------
...
def **repr**(self) -> str:
# FIX 1 + 9: was self._call_count (undefined). Use calls_made from BaseAdvisor.
