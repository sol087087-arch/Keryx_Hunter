...
    is_essential: bool = False # never trimmed
    error_count: int = 0
...
# SharedContext
# ---------------------------------------------------------------------------
# These are instance-level defaults, not class-level constants,
# FIX 7: avoid shared mutable state between test instances.
...
        FIX 2: steps_taken is incremented ONLY inside add_step().
        agent.py must NOT do context.steps_taken += 1 separately.
        Agent tracks its loop iteration independently if needed.
    Advisor advice lifecycle:
        add_advisor_advice(advice) — store pending advice
        has_pending_advisor_advice() — check without consuming
        peek_advisor_advice() — read without consuming
        consume_advisor_advice() — read and clear (use this)
        clear_pending_advisor_advice() — explicit clear (agent.py contract)
    Stupid loop detection:
        FIX 3: counters are per-action-signature, not global.
        codeql succeeding does NOT reset gdb's failure counter.
    """
    def **init**(
        self,
        target_path: str,
...
        # FIX 7: instance-level thresholds, not class-level
...
        # FIX 3: per-action-signature failure counters (not global)
...
        """
        Record one ReAct step.
        FIX 2: steps_taken incremented HERE only.
        agent.py must NOT also do context.steps_taken += 1.
        """
...
        ))
        # FIX 3: per-signature counters
        sig = f"{name}:{json.dumps(inp, sort_keys=True, default=str)}"
        is_error = any(k in observation.lower() for k in ("error", "fail", "timeout", "crash"))
        if is_error:
...
                )
        else:
            # Success resets only THIS action's counter
...
        self.steps_taken += 1 # FIX 2: single increment point
        if name == "read_file":
...
        """Call after advisor intervention breaks the loop."""
...
    ) -> Tuple[bool, str]:
        """
        FIX 4: confirmed_vulns use 'action' key, not 'file'.
        Use 'observation' snippet as confirmed-file proxy, or check by step_num.
        Simplified: just check if any file has spent > threshold with zero confirmations.
        """
        thr = threshold or self._cost_threshold
        # Which files are covered by a confirmed vuln?
...
            if cost <= thr:
                continue
            # Rough check: is this file mentioned in any confirmed observation?
...
        """
        Trim history preserving essential and recent steps.
        FIX 6: cap the total result at keep_last to prevent prompt growth.
        We preserve essential steps WITHIN keep_last budget, not on top of it.
        """
...
        # Essential steps that aren't already in recent — fill remaining budget
...
        """Called after advisor escalation; resets parse error cooldown."""
        self.parse_errors = 0
...
    # ------------------------------------------------------------------
    # Advisor state — explicit consume/peek API
    # ------------------------------------------------------------------
...
        """Read without consuming. Use for checking content."""
        return self._pending_advisor_advice
    def consume_advisor_advice(self) -> Optional[Any]:
        """
        FIX 1+5: Read AND clear in one atomic operation.
        Preferred over get_pending then clear_pending separately.
        """
...
        """
        Alias for consume_advisor_advice() — matches agent.py call pattern.
        Clears on read. Use peek_advisor_advice() if you need non-consuming read.
        """
        return self.consume_advisor_advice()
    def clear_pending_advisor_advice(self) -> None:
        """
        FIX 9: explicit clear — called by agent.py _apply_advisor_advice().
        Works correctly even if consume was already called.
        """
        self._pending_advisor_advice = None
...
    # ------------------------------------------------------------------
    # Knowledge graph fragment (for advisors)
    # ------------------------------------------------------------------
...
            # pending_advisor_advice intentionally omitted — session-specific
        }
    @classmethod
...
        """
        FIX 8: exclude session_cost_usd — it changes every API call,
        making the hash unstable as a cache key.
        Use structural state only: path, steps, confirmed count.
        """
